...
public class ObstacleSpawnerJ : MonoBehaviour
{
    ...
    void OnEnable()
    {
        if (GameManagerJ.Instance != null)
        {
            GameManagerJ.Instance.OnGameRestarted += ResetSpawner; // Tambahkan event restart dari GameManager
        }
    ...
    void OnDisable()
    {
        if (GameManagerJ.Instance != null)
        {
            GameManagerJ.Instance.OnGameRestarted -= ResetSpawner;
        }
        ObstacleMove.OnObstacleReachedEnd -= OnPreviousObstacleCleared;
    }
    ...
    void Update()
    {
        ...
        if (GameManagerJ.Instance == null || !GameManagerJ.Instance.IsGameStarted)
            return;
